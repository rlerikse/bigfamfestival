...
3. **Verify project setup** (create/update .gitignore, .dockerignore based on tech stack)
4. **Execute tasks** (phase-by-phase, mark completed)
5. **Run validation** (after each phase: tests pass, linting clean, type checks)
6. **Report completion** (summary with next steps)
...
│                                                              │
│ EXECUTION FLOW:                                             │
│ 1. Checklist validation (ask if incomplete)                 │
│ 2. Project setup (ignore files, dependencies)               │
│ 3. Execute phases (Setup → Foundational → User Stories)    │
│ 4. Mark tasks complete (track progress)                     │
│ 5. Validate each phase (tests, linting)                     │
│ 6. Report completion (summary + next steps)                 │
└─────────────────────────────────────────────────────────────┘
```
...
10. **Update workspace context**: After implementation is complete, run `/speckit.context` to update workspace context files reflecting the completed feature.
