...
### [2026-08-09] Human release-notes drift: NEXT.md stale, no versioned files since 1.2.2
`mobile/release-notes/` contains only `1.2.2.md` + `NEXT.md`, but the app has shipped through `v1.4.2` (all 4 manifests + tag consistent at 1.4.2). No versioned release-notes files exist for 1.3.x–1.4.2, and `NEXT.md` still holds already-released friend-location/map content (shipped in 1.4.x) — the `NEXT.md → <version>.md` roll-over hasn't been done since 1.2.2. Note this is HUMAN release notes, separate from release-please's technical `CHANGELOG.md` (which is current). Surfaced by the first `/postmerge` run. Backlog cleanup (not yet done): create versioned release-notes files for past releases and/or roll NEXT.md so it only holds unreleased work. `/postmerge` keeps NEXT.md current going forward but does not retro-reconstruct the missing versioned files.
— discovered during first /postmerge run
